...
const SearchStaffTickets = ({ hospitalId }) => {
  ...
  const handleSubmit = async () => {
    ...
    await searchTicket({ search: search })
      .then((data) => {
        let array = [];
        console.log(data.data[0].hospitals[0]);
        data.data.map((app) => {
          if (app.hospitals[0]._id == hospitalId) {
            array.push(app);
          }
        });
        console.log(data);
  ...
  return (
    <>
      <div className="col-span-3">
        ...
        />
        <div className="absolute z-100   md:h-auto pb-10 bg-gray-200 mr-10 rounded w-auto">
...
export default SearchStaffTickets;
